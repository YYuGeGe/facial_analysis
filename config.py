def create_circular_connections(point_list):
    """
    将给定的点列表构建成闭合轮廓的连接关系（每两个相邻点为一条边，并连接首尾）

    Args:
        point_list (list): 包含关键点索引的列表
    ...
    # 添加第一个点到最后以实现首尾相连
    circular_points = point_list + [point_list[0]]

    # 构建连接线：每两个相邻点组成一条边
    connections = [
        (circular_points[i], circular_points[i + 1])
        for i in range(0, len(circular_points) - 1)
    ]

    return frozenset(connections)


# data_root_path = r'C:\Users\Administrator\Desktop\BTZJ\images2025-07-08'
data_root_path = r'F:\Desktop\CocoPi\btzj\images2025-07-08'


extensions = ['.jpg', '.jpeg', '.png', '.tif', '.tiff', '.bmp']

lip_dilate_size = 7
lip_dilate_iterations = 4


min_red_area = 4

mouse_intensity_threshold = 110
mouse_red_threshold = 100
mouse_green_threshold = 150
mouse_blue_threshold = 150

nose_intensity_threshold = 110
nose_red_threshold = 100
nose_green_threshold = 150
nose_blue_threshold = 150

# ======================
head_intensity_threshold = 95
head_red_threshold = 90
head_green_threshold = 100
head_blue_threshold = 100

right_face_intensity_threshold = 100
right_face_red_threshold = 120
right_face_green_threshold = 100
right_face_blue_threshold = 100

left_face_intensity_threshold = 100
left_face_red_threshold = 120
left_face_green_threshold = 100
left_face_blue_threshold = 100


mouse_shape = [360, 315]
nose_shape = [200, 300]
head_shape = [600, 166]
right_face_shape = [272, 544]
left_face_shape = [272, 544]
...
#  嘴巴外轮廓
lip_points = [61, 185, 40, 39, 37, 0, 267, 269, 270, 409, 291, 375, 321, 405, 314, 17, 84, 181, 91, 146]
mouse_points = [94, 97, 98,
               203, 206, 216, 212, 210, 169, 150,
               149, 176, 148, 152, 377, 400, 378, 379,
               394, 430, 432, 436, 426, 423, 327, 326]

#  鼻子轮廓
nose_points = [94, 141, 97, 98, 129, 49, 131, 198, 188, 245, 189, 221, 55, 8, 285, 441, 413, 465, 412, 420, 360, 279,
              358, 327, 326, 370, 94]

# 额头
head_points = [9, 107, 66, 105, 63, 70, 71, 21,
              21, 54, 103, 67, 109, 10, 338, 297, 332, 284, 251,
              301, 300, 293, 334, 296, 336]

# 右脸
right_face_points = [188, 245, 128, 232, 231, 230, 229, 228, 31, 35, 124, 70, 71,
                    21, 162, 127, 234, 93, 132, 58, 172, 136, 150,
                    169, 210, 212, 216, 206, 203, 98, 129, 49, 131, 198]

#  左脸
left_face_points = [412, 465, 357, 452, 252, 253, 254, 339, 255, 446, 353, 300, 301,
                   251, 389, 356, 454, 323, 361, 288, 397, 365,
                   379, 394, 430, 432, 436, 426, 423, 327, 358, 279, 360, 420]


lip_connections = create_circular_connections(lip_points)
mouse_connections = create_circular_connections(mouse_points)

nose_connections = create_circular_connections(nose_points)
head_connections = create_circular_connections(head_points)

right_face_connections = create_circular_connections(right_face_points)
left_face_connections = create_circular_connections(left_face_points)

all_connections = frozenset().union(*[lip_connections, mouse_connections,
                                      nose_connections, head_connections,
                                      right_face_connections, left_face_connections])
